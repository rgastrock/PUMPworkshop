...
Seems about alright, but it's not that pretty... Now-a-days most people make graphs using `ggplot2`, but we won't talk about that here. Here's a sneak-peek of what that would look like
...
We re-run the code for the graph, but we add something extra before and after the code:
...
dev.off() # Say that we're done with the file
```
